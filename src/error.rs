...
pub(crate) fn short_circuit_current<U: Debug, V: Debug, T>(id: &Id,
                                                           current: U,
                                                           impedance: &RatioFrac<V>)
                                                           -> Result<T>
{
	Err(Error::CircuitSolveError(format!("Short circuit is caused by a non zero constant current \
	                                      source on a zero admittance component\n\nComponent Id: \
	                                      {:?}\nCurrent: {:?} A\nImpedance (rational fraction of \
	                                      pulse):\n{:?}\n-------\n{:?}",
	                                     id, current, impedance.numerator, impedance.denominator)))
}

pub(crate) fn short_circuit_tension<U: Debug, V: Debug, T>(id: &Id,
                                                           tension: U,
                                                           impedance: &RatioFrac<V>)
                                                           -> Result<T>
{
	Err(Error::CircuitSolveError(format!("Short circuit is caused by a non zero constant tension \
	                                      source on a zero impedance component\n\nComponent Id: \
	                                      {:?}\nTension: {:?} V\nImpedance (rational fraction of \
	                                      pulse):\n{:?}\n-------\n{:?}",
