fn test_() -> crate::error::Result<()>
{
	...
	let mut chart = ChartBuilder::on(&root).caption("Tensions", ("sans-serif", 20).into_font())
	                                       .margin(5)
	...
	chart.draw_series(LineSeries::new(time_array.iter().zip(tensions1.iter()).map(|(x, y)| (*x, *y)), &RED))
	     .expect("Failed to draw the source's tensions")
	     .label("Tension generated by the source")
	     .legend(|(x, y)| PathElement::new(vec![(x, y), (x + 20, y)], &RED));
	chart.draw_series(LineSeries::new(time_array.iter().zip(tensions2.iter()).map(|(x, y)| (*x, *y)), &BLUE))
